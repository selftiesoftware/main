 ...
 */
case class Circle(center : Vector, p : Vector) extends EnclosedGeometry
{
  
  lazy val radius = (center - p).length
  ...
  def distanceTo(point : Vector) =
    scala.math.abs(radius - (center - point).length)

  def handles = List(Vector(center.x + radius, center.y),
   ...
   */
  def intersect(line : Segment) = intersects(line).size > 0
   ...
   * Source: http://local.wasp.uwa.edu.au/~pbourke/geometry/2circle/
   */
  def intersects(circle : Circle) : Seq[Vector] = if (intersect(circle)) {
      ...
      if (d == radius + circle.radius) {
        ...
        // Return
        Seq(firstSolution, secondSolution)
      }
    } else Seq()
   ...
   */
  def intersects(line : Segment) = line.intersects(this)
