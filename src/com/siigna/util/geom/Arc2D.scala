 ...
 */
case class Arc2D(center : Vector2D, radius : Double, startAngle : Double, angle : Double) extends Arc[Dimension2] {
   ...
   */
  lazy val length = {
    val circumference : Double = radius * radius * math.Pi
    circumference * (angle / 360.0)
  }
