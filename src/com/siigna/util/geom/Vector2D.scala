 ...
 */
case class Vector2D(x : Double, y : Double) extends Vector[Dimension2]
{
   ...
   */
  def transform(transformation : TransformationMatrix) : Vector2D = transformation.transform(this)
