 ...
 */
case class Segment(p1 : Vector, p2 : Vector) extends BasicGeometry
{
   ...
   */
  def distanceTo(point : Vector) : Double = (point - closestPointTo(point)).length
   ...
   */
  def closestPointTo(point: Vector) = {
   ...
   */
  def intersect(arc : Arc) =
  {
    val circle = Circle(arc.center, arc.start)
    val intersections = intersects(circle)
