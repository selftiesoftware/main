 ...
 *
 * TODO: Refactor the imageDimension to another type than rectangle.
 */
case class ImageShape(image : Array[Int], p1 : Vector, p2 : Vector, width : Int, height : Int, attributes : Attributes) extends EnclosedShape {

  val geometry = new Rectangle(p1, p2)
  ...
  /**
   * Retreives the ImageShape as an Image.
   */
  def toImage : Image = {
...
object ImageShape {
  ...
  def apply(image : BufferedImage, p1 : Vector, p2 : Vector) : ImageShape =
    fromImage(image, p1, p2, image.getWidth, image.getHeight)

  def apply(image : Array[Int], p1 : Vector, p2 : Vector, width : Int, height : Int) : ImageShape =
    ImageShape(image, p1, p2, width, height, Attributes())

  def fromImage(image : Image, p1 : Vector, p2 : Vector) : ImageShape =
    fromImage(image, p1, p2, image.getWidth(null), image.getHeight(null))

  def fromImage(image : Image, p1 : Vector, p2 : Vector, width : Int, height : Int) : ImageShape =
    fromImage(image, p1, p2, width, height, Attributes())
  
  /**
   * Creates an ImageShape from a given image.
   */
  def fromImage(highResImage : Image, p1 : Vector, p2 : Vector, width : Int, height : Int, attributes : Attributes) : ImageShape = {
