 ...
 */
case class ArcShape(start : Vector, middle : Vector, end : Vector, attributes : Attributes) extends BasicShape
{
...
object ArcShape
{
   ...
   * @param end  The vector where the arc stops (CCW).
   */
  def apply(start : Vector, middle : Vector, end : Vector) = new ArcShape(start, middle, end, Attributes())
   ...
   */
  def apply(center : Vector, radius : Double, startAngle : Double, endAngle : Double) = {
