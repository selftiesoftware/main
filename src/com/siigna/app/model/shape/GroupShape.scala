 ...
 */
case class GroupShape(ids : Seq[String], attributes : Attributes) extends Shape
        with collection.generic.SeqForwarder[ImmutableShape]
{
   ...
   */
  def distanceTo(point : Vector, scale : Double) : Double = shapes.map(_.distanceTo(point, scale)).reduceLeft(math.min)
