...
trait ImmutableShape extends Shape {
   ...
   */
  override def attributes_=(attributes : Attributes) = setAttributes(attributes)
   ...
   */
  override def attributes_+=(attribute : (String, Any)) = attributes += attribute
   ...
   */
  override def attributes_+=(attributes : (String, Any)*) = attributes_=(this.attributes ++ attributes)

  /**
   * Calculates the closest distance to the shape in the given scale.
   */
  def distanceTo(point : Vector, scale : Double) = geometry.distanceTo(point) * scale
...
trait EnclosedShape extends ImmutableShape {
  ...
  override def geometry : EnclosedGeometry
