...
trait ImmutableShape extends Shape {
   ...
   */
  def select() : TransformationMatrix => ImmutableShape = transform(_)
   ...
   */
  def select(rect : Rectangle2D) : Option[TransformationMatrix => ImmutableShape]
   ...
   */
  def select(point : Vector2D) : Option[TransformationMatrix => ImmutableShape] // select shape close to the point
