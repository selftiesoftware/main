 ...
 */
class JarLoader(private var urls : Array[URL], loader : ClassLoader) {
      ...
      val result : Option[Module] = if (resource.isEmpty) None else try {
        ...
        case e : ExceptionInInitializerError => {
          ...
          try {
            ...
            case err => errors += ("JarLoader: Module " + classPath + " was found but could not be initialized through MODULE$ field or constructor." -> Some(err))
            None
          }
        }
        case e => errors += ("JarLoader: Class found, but failed to cast to Module." -> Some(e)); None
      }
