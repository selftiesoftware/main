...
/**
 * A simple rectangle given by two points. Contrary to a [[com.siigna.util.geom.Rectangle2D]] a simple rectangle
 * cannot be rotated.
 *
...
case class SimpleRectangle2D(xMin : Double, yMin : Double, xMax : Double, yMax : Double) extends Rectangle with GeometryClosed2D {
  ...
  def expand(geom : Geometry2D) : SimpleRectangle2D = geom match {
     ...
     */
    case circle : Circle2D =>
      expand(SimpleRectangle2D(Vector(circle.center.x - circle.radius, circle.center.y + circle.radius),
    ...
    case rect : SimpleRectangle2D => {
      ...
      SimpleRectangle2D(newTopLeft, newBottomRight)
    }
...
/**
 * A companion object to SimpleRectangle2D.
 */
object SimpleRectangle2D {
